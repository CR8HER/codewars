...
function passTheDoorMan (word){
    ...
    // if alpha index num of char matches its neighbor in the word string, extract its index value (alpha) and mulitply by 3
    
    for (let i = 0; i < word.length - 1; i++) {
      if (alphabet.indexOf(word[i]) === alphabet.indexOf(word[i+1])) {
        return alphabet.indexOf(word[i]) * 3    
      }
    }
